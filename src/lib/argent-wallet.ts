...
const getERC20Tokens = async (address: string) => {
  const erc20FetchBalance = `http://api.etherscan.io/api?module=account&action=tokentx&address=${address}&startblock=0&endblock=999999999&sort=asc&apikey=YourApiKeyToken`
  const {
    body: { result },
